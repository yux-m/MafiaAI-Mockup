    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "A user requests a joke. Say a funny joke about %s."
                    % word,
                }
            ],
            temperature=1,
...
async def game_end(
    ...
    # print("player objects", server.players.values())
    ...
    await channel.send(
        "\n".join(
            ...
            + [
                "<@%s> : `%s`" % (server.id_to_player[player.id], player.role)
...
async def daytime(channel, server):
    ...
    if not (
        server.settings["daystart"] and server.round == 1
    ):  # night actions were taken
        ...
        for player in server.players.values():
            if player.alive and player.role == "normalcop":  # normal cop report
                ...
                if player.cur_choice is None:
                    await user.send(
                        "You inquired about nobody, and so you receive no report."
                    )
                else:
                    target = await openai.fetch_user(player.cur_choice.id)
                    await user.send(
                        "You received a report that **%s** is %s."
                        % (
                            target.name,
                            ["innocent", "guilty"][player.cur_choice.role == "mafia"],
                        )
                    )
                ...
                if player.cur_choice == None:
                    ...
                    target = await openai.fetch_user(player.cur_choice.id)
                    await user.send(
                        "Your next target will be compared to **%s**, and you will determine whether their alignments are the same or not."
                        % target.name
                    )
                else:
                    ...
                    if [player.lst_choice.role, player.cur_choice.role].count(
                        "mafia"
                    ) == 1 or player.lst_choice.role != player.cur_choice.role:
                        await user.send(
                            "You received a report that **%s** and **%s** are of different alignments."
                            % (lst.name, cur.name)
                        )
                    else:
                        await user.send(
                            "You received a report that **%s** and **%s** are of the same alignment."
                            % (lst.name, cur.name)
                        )
            elif (
                player.alive and player.role == "doctor" and player.action
            ):  # doctor save
    ...
    await channel.send(
        "It is day %d! You have %s minutes to decide upon a lynch."
        % (server.round, str(server.settings["limit1"]))
    )
    ...
    while (
        ...
        or (time.time() - start_time) < server.settings["limit1"] * 60
    ) and server.running:
        if server.time != "inf":
...
async def get_options(player, server):
    ...
    for p in server.players.values():
        if (
            p == player
            and not (player.role == "doctor" and server.settings["selfsave"])
            and not player.role == "paritycop"
        ):
            continue
...
async def output_options(player, server):
    ...
    await user.send(
        "\n".join(
            ["%d - **%s**" % (option[0], option[1].name) for option in player.options]
        )
    )
...
async def maf_options(mafias, server):
    ...
    for player in mafias:
        ...
        if len(mafias) > 1:
            await user.send(
                "The other remaining mafia are:\n"
                + "\n".join(
                    [
                        "**%s**" % (await openai.fetch_user(mafia.id)).name
                        for mafia in mafias
                        if mafia != player
                    ]
                )
...
async def m_ncop(player, server, choice):
    ...
    await user.send(
        "You have selected **%s** as the target of your investigation. You will recieve a report in the morning."
        % target
    )
    await user.send(
        "You may change your choice as long as not everyone has completed their night action."
    )
...
async def m_pcop(player, server, choice):
    ...
    if not player.lst_choice:
        await user.send(
            "You have selected **%s** as the target of your investigation. Remember that you will not recieve a report in the morning, as you are a parity cop."
            % target
        )
    else:
        lst = await openai.fetch_user(player.lst_choice.id)
...
async def m_doc(player, server, choice):
    ...
    target = player.options[choice][1].name
    await user.send(
        "You have selected **%s** as the target of your save. They will be immune to death tonight."
        % target
    )
    await user.send(
        "You may change your choice as long as not everyone has completed their night action."
...
async def m_maf(player, server, choice):
    ...
    if maj == None:
        ...
        await user.send(
            "There is presently a majority vote to kill **%s**."
            % (await openai.fetch_user(maj.id)).name
        )
    ...
    for p in server.players.values():
        ...
        await maf_user.send(
            "**%s** has selected **%s** as their target to kill." % (user.name, target)
        )
        if maj == None:
            await user.send("There is presently no majority in your votes.")
        elif maj == "no-kill":
...
pr_funcs = {"normalcop": m_ncop, "paritycop": m_pcop, "doctor": m_doc, "mafia": m_maf}
...
async def nighttime(channel, server):
    ...
    await channel.send(
        "It is now night %d. If you have a nighttime action, you have %s minutes to take it."
    ...
    server.actions = sum(
        [
            player.alive and player.role in power_roles
            for player in server.players.values()
        ]
    )
    server.saves = []
    ...
    while (
        (
            ...
            or (time.time() - start_time) < server.settings["limit2"] * 60
        )
        and server.running
        and server.actions
    ):
...
):  # can only end game if currently playing (alive) or server mod/admin
    if not server.running:
        await message.channel.send("There is no ongoing game to end.")
        return
    if message.author.guild_permissions.administrator or (
        author in server.players and server.players[author].alive
    ):
...
async def m_set(message, author, server):
    ...
    if num != int(num):
        ...
        await message.channel.send(
            "Invalid input: inputted quantity cannot be negative."
        )
    else:
        num = int(num)
        server.setup[query[1]] = num
        await message.channel.send(
            "Successfully changed the number of `%ss` in the setup to `%d`."
            % (query[1], num)
        )
...
async def m_settings(message, author, server):
    msg = [
        "%s : %d - %s"
        % (key, server.settings[key], toggle_text[server.settings[key]][key])
        for key in toggle_text[0]
    ]
    msg += [
        "Time limit for %s is %s minute(s)."
        % (["days", "nights"][x - 1], server.settings["limit" + str(x)])
        for x in [1, 2]
    ]
    await message.channel.send("\n".join(msg))
    ...
    if query[2] == "inf":
        ...
        try:
            ...
            if query[1] == "day":
                ...
                await message.channel.send(
                    "Time limit for day set to " + query[2] + " minutes."
                )
            else:
                server.settings["limit2"] = lim
                await message.channel.send(
                    "Time limit for night set to " + query[2] + " minutes."
                )
        except ValueError:
            await invalid(message)
...
async def m_join(message, author, server):
    if server.running:
        await message.channel.send(
            "Game is ongoing: please wait until it ends before joining."
        )
        return
    if author in server.players:
        ...
        await message.channel.send(
            "<@%s>, you cannot be in more than one game at a time!" % str(author)
        )
    else:
...
async def m_leave(message, author, server):
    if (
        author not in allPlayers or allPlayers[author] != message.guild
    ):  # not server they're playing in
        await message.channel.send(
            "<@%s>, you are not currently part of the game in this server."
            % str(author)
        )
        return
    if server.running and author in server.players:
        ...
        if server.players[author].alive:
            ...
            if server.settings["continue"]:
                ...
                if (
                    not server.phase
                    and server.players[author].role in power_roles
                    and not server.players[author].action
                ):  # if nighttime and power role unfulfilled
                    server.actions -= 1
    ...
    # TODO Removes from predictorAI and Server object as well.
...
async def m_vote(message, author, server):
    if not server.running:
        await message.channel.send(
            "The game has not yet started. Don't be so hasty to vote!"
        )
        return
    if (
        author not in server.players
        or not server.players[author].alive
        or not server.phase
    ):  # not playing, not alive, night
    ...
    try:
        if (
            ...
            or tar[-1] != ">"
            or (
                int(tar[2:-1]) != openai.user.id
                and int(tar[2:-1]) not in server.players
            )
            or (
                int(tar[2:-1]) in server.players
                and not server.players[int(tar[2:-1])].alive
            )
        ):
            await message.channel.send(
        ...
        await message.channel.send(
            "That is an invalid voting target. Vote in the form `m!vote @user`."
        )
        return
    await message.channel.send(
...
async def m_unvote(message, author, server):
    if not server.running:
        await message.channel.send(
            "The game has not yet started. There's nobody to unvote!"
        )
        return
    if (
        author not in server.players
    ...
    await message.channel.send(
        "<@%s> has removed their vote, and is now voting nobody." % str(author)
    )
...
async def m_status(message, author, server):
    if not server.running:
        await message.channel.send(
            "The game has not yet started. There are no votes in effect."
        )
        return
    if (
        author not in server.players
        ...
        await message.channel.send(
            "Daytime is not in session. There are no votes in effect."
        )
        return
    ...
    for player in server.players.values():
        ...
        if player.vote == openai.user.id:
            ...
            msg.append(
                "<@%s> is currently voting <@%s>" % (str(player.id), player.vote)
            )
    ...
    for i in range(num, -1, -1):
        if count2[i]:
            msg.append(
                str(i)
                + " vote(s) on: "
                + ", ".join(["<@%s>" % str(key) for key in count2[i]])
            )
    ...
    msg.append(
        "Nobody: %d vote(s)"
        % sum(
            [player.vote == None for player in server.players.values() if player.alive]
        )
    )
...
async def m_players(message, author, server):
    ...
    if not server.running:
        if not num:
            await message.channel.send(
                "There are currently no players in the game. Type `m!join` to join!"
            )
        else:
            await message.channel.send(
                " ".join(
                    ["The following players are in the game:"]
                    + ["<@%s>" % str(key) for key in server.players]
                )
            )
        return
    await message.channel.send(
...
async def m_alive(message, author, server):
    if not server.running:
        await message.channel.send(
            "There is no ongoing game. Use `m!setup` to see the current setup of the game."
        )
        return
    if not server.settings["reveal"]:
        await message.channel.send(
            "Remaining roles are unknown, due to the `reveal` setting being toggled off."
        )
    else:
...
async def m_dead(message, author, server):
    if not server.running:
        await message.channel.send(
            "There is no ongoing game. Use `m!players` to check who's planning to play."
        )
        return
    if not sum([player.alive == 0 for player in server.players.values()]):
        await message.channel.send(
            "The graveyard is currently empty. Not for long, though..."
        )
        return
    if not server.settings["reveal"]:
        await message.channel.send(
            " ".join(
                ["The graveyard consists of:"]
                + [
                    "<@%s>" % str(player.id)
                    for player in server.players.values()
                    if not player.alive
        ...
        await message.channel.send(
            "\n".join(
                ["The graveyard consists of:"]
                + [
                    "<@%s>, who was a %s" % (str(player.id), player.role)
                    for player in server.players.values()
                    if not player.alive()
                ]
            )
...
async def m_time(message, author, server):
    ...
    if server.phase:
        ...
        if server.settings["limit2"] == "inf":
            ...
            await message.channel.send(
                "There are %d minutes and %d seconds remaining in the night."
                % (int(server.time) / 60, int(server.time) % 60)
            )
